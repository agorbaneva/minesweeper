...
public class Game extends JFrame implements KeyListener{
    ...
    public void setMines() {
    	...
    	while(count < mines) {
    		...
    		if(!(cells[yCord][xCord].getMine() || (xCord == 1 && yCord == boardHeight))) {
    ...
    public void revealZeros(int x, int y) {
		...
		for(int xDif = -1; xDif <= 1; xDif ++) {
			for(int yDif = -1; yDif <= 1; yDif ++) {
				if(!((x + xDif < 0) || (y + yDif < 0) || (x + xDif > boardWidth - 1) || (y + yDif > boardHeight - 1)) && !finalCells[y + yDif][x + xDif].isRevealed()) {
