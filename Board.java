...
public class Board extends JPanel {
  ...
  public void paintComponent(Graphics g) {
    ...
    g.drawString(
      Integer.toString(minesweeper.getFlagCount()),
      2 * blockSize + blockSize / 8,
      3 * blockSize / 8 + blockSize
    );

    //time count
    imageIcons
      .get("clock")
      .paintIcon(this, g, blockSize * 8 + blockSize / 2, blockSize / 2);
    if (!minesweeper.getFinished()) {
      sec = minesweeper.getTime();
    }
    g.drawString(
      Integer.toString(sec),
      9 * blockSize + 3 * blockSize / 4,
      3 * blockSize / 8 + blockSize
    );
    ...
    imageIcons
      .get("trophy")
      .paintIcon(this, g, blockSize * 15, blockSize / 2 + 1);
    try {
    ...
    g.drawString(
      Integer.toString(bestTime),
      16 * blockSize + blockSize / 8,
      3 * blockSize / 8 + blockSize
    );
  }
  ...
  public Color getBackground(int x, int y, boolean isRevealed) {
    Color c;
    if (isRevealed) {
      if ((x + y) % 2 == 0) c = revealed1; else c = revealed2;
    } else {
      if ((x + y) % 2 == 0) c = background1; else c = background2;
    }
    return c;
  }
